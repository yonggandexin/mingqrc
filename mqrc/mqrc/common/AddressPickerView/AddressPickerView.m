...
@property (nonatomic ,strong) NSArray   * dataArr;/**< 省市区数据源字典*/
...
- (UIView *)titleBackgroundView{
    if (!_titleBackgroundView) {
        ...
        line.frame = CGRectMake(0, 0, SELFSIZE.width, 0.5);
        line.backgroundColor = [UIColor lightGrayColor];
        [_titleBackgroundView addSubview:line];
    }
...
- (void)loadAddressData{
    ...
    _dataArr = [self dictionaryWithJsonString:str22];
...
numberOfRowsInComponent:(NSInteger)component{
    ...
    if (0 == component) {
        ...
        Province  * p            = _pArr[selectProvince];
        if (selectCity > p.Citys.count - 1) {
            return 0;
        }
        City * c = p.Citys[selectCity];
...
- (NSString *)pickerView:(UIPickerView *)pickerView
    ...
    if (0 == component) {
        ...
        if (row > selectP.Citys.count - 1) {
            return nil;
        }
        City *c = selectP.Citys[row];
        return c.Name;
    }
    else if (2 == component) {
        NSInteger selectProvince = [pickerView selectedRowInComponent:0];
...
- (void)showOrHide:(BOOL)isShow{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        if (isShow) {
            selfkY = [UIScreen mainScreen].bounds.size.height - 205-nav_barH;
        }
        else {
            selfkY = [UIScreen mainScreen].bounds.size.height-nav_barH;
        }
        self.frame = CGRectMake(0,selfkY, self.bounds.size.width,205);
