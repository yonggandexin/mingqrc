...
+ (void)showGifLoadToView:(UIView *)view{
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:view animated:YES];
    //使用SDWebImage 放入gif 图片
    UIImage *image = [UIImage sd_animatedGIFNamed:@"loading1"];
    //自定义imageView
    UIImageView *cusImageV = [[UIImageView alloc] initWithImage:image];
    cusImageV.backgroundColor = [UIColor whiteColor];
    cusImageV.frame = CGRectMake(0, 0, 150, 60);
    //设置hud模式
...
+ (void)showAutoMessage:(NSString *)message ToView:(UIView *)view{
    [self showMessage:message ToView:view RemainTime:2.5 Model:MBProgressHUDModeText];
}
