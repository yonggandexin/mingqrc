...
- (void)addLocal {
    ...
    CGFloat top = IS_IPHONE_X?15:0;
    UIImageView *imgV = [[UIImageView alloc]initWithFrame:CGRectMake(20, 30+top, 12, 15)];
    imgV.image = [UIImage imageNamed:@"WhiteLocation"];
    [self addSubview:imgV];
    
    
    _loaclBtn = [UIButton buttonWithType: UIButtonTypeCustom];
    _loaclBtn.frame =  CGRectMake(CGRectGetMaxX(imgV.frame)+3, 23+top, 120, 25);
    NSString *province = [LocaltionInstance shareInstance].province;
    [_loaclBtn setTitle:province forState:UIControlStateNormal];
    [_loaclBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    _loaclBtn.titleLabel.font = font(14);
    [_loaclBtn addTarget:self action:@selector(localBtnClicked) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:_loaclBtn];
    [_loaclBtn sizeToFit];
    ...
    newBtn.frame = CGRectMake(SCREEN_WIDTH-50, 20+top, 40, 30);
    [newBtn setImage:[UIImage imageNamed:@"news"] forState:UIControlStateNormal];
...
{
    if ([_loaclBtn.titleLabel.text isEqualToString:@"定位失败"]) {
        [_loaclBtn setTitle:@"定位中..." forState:UIControlStateNormal];
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(scanlMoreNews)]) {
        [_delegate scanlMoreNews];
    }
}

- (void)relocation
{
    [_loaclBtn setTitle:[LocaltionInstance shareInstance].province forState:UIControlStateNormal];
}
...
- (void)changeAlphaWithCurrentOffset:(CGFloat)offsetY {
    ...
    CGFloat toTop = IS_IPHONE_X?-44:-20;
    if (offsetY<=toTop) {
        self.y = fabs(offsetY)-fabs(toTop)-1.5;
    }
