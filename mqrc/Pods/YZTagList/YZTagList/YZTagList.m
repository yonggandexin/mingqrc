...
//删除多个标签
- (void)deleteTags:(NSArray *)tagStrs
{
    for (NSString *tagStr in tagStrs) {
        [self deleteTag:tagStr];
    }
}
...
- (NSInteger)tagButs
{
    ...
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[YZTagButton class]]) {
            [btns addObject:view];
        }
    }
...
- (void)deleteAllTags
{
    [self.tags enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        [self deleteTag:key];
    }];
